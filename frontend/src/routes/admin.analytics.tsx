...
import { IndianRupee, Users, Store, Package, Receipt, Warehouse, Sparkles, TrendingUp, Download, RefreshCw, Calendar } from "lucide-react";
...
const searchSchema = z.object({
  ...
  vendor: z.string().optional().catch("all"),
...
export const Route = createFileRoute("/admin/analytics")({
  ...
  loader: async ({ search = {} as any }) => {
    ...
    if (search.vendor && search.vendor !== "all") params.set("vendor", search.vendor);
    ...
    // Also fetch vendors & customers list for select dropdown filters
    const vendorsRes: any = await api.get("/vendors?limit=100");
    
    return {
      analytics: res.data,
      vendorsList: vendorsRes.data.vendors || [],
    };
  },
...
function AdminAnalyticsDashboard() {
  ...
  const handleExportCSV = () => {
    try {
      ...
      link.setAttribute("download", `ShopSense_AnalyticsDashboard_${new Date().toISOString().split("T")[0]}.csv`);
  ...
  const handleExportExcel = () => {
    try {
      ...
      <head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>ShopSense Report</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head>
      <body>
        <table border="1">
          <thead>
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `ShopSense_AnalyticsDashboard_${new Date().toISOString().split("T")[0]}.xls`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  const catDistribution = analytics.product?.categoryDistribution || [];

  const vendorPerformance = useMemo(() => {
    if (analytics.mlBenchmarking?.rankings) {
      return analytics.mlBenchmarking.rankings.map((r: any) => ({
        businessName: r.business_name,
        revenue: r.revenue,
        orders: r.orders,
      })).slice(0, 5);
    }
    return (analytics.vendor?.performance || []).slice(0, 5);
  }, [analytics.mlBenchmarking, analytics.vendor]);
  ...
  const forecastTrend = (analytics.forecast?.forecastTrend || []).map((t: any) => ({
    date: t.date,
  ...
  const vendorColumns = useMemo<ColumnDef<any>[]>(
    () => [
      {
        accessorKey: "businessName",
        header: "Vendor",
        cell: ({ row }) => <span className="font-semibold text-xs">{row.original.businessName}</span>,
      },
      {
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader
        title="Centralized Analytics Console"
        description="Unified corporate statistics dashboard providing revenue paths, customer segments distribution, inventory turnovers, and recommendations coverage."
        actions={
          <>
            <DropdownMenu>
            ...
            <Button variant="ghost" size="icon" className="print:hidden" onClick={async () => {
              ...
              toast.success("Dashboard analytics data updated");
            }}>
      ...
      <Card className="p-4 bg-muted/20 border-dashed space-y-4">
        <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 md:grid-cols-5 items-end">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground">Select Vendor</label>
            <Select
              value={searchParams.vendor}
              onValueChange={(v) => updateQuery({ vendor: v })}
            >
              <SelectTrigger>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        <StatCard label="Total revenue" value={inr(summary.totalRevenue || 0)} icon={IndianRupee} tone="emerald" />
        <StatCard label="Products Sold" value={num(summary.productsSold || 0)} icon={Package} tone="primary" />
        <StatCard label="Active customers" value={num(summary.totalCustomers || 0)} icon={Users} tone="accent" />
        <StatCard label="Total vendors" value={num(summary.totalVendors || 0)} icon={Store} tone="info" />
        <StatCard label="Total transactions" value={num(summary.totalTransactions || 0)} icon={Receipt} tone="primary" />
        <StatCard label="Inventory items" value={num(summary.totalInventoryItems || 0)} icon={Warehouse} tone="accent" />
        ...
        <StatCard label="Low/Out of Stock Items" value={`${summary.lowStockItems || 0} / ${summary.outOfStockItems || 0}`} icon={Warehouse} tone="warning" />
      </div>
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-2">
        ...
        <Card className="p-5 flex flex-col justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground mb-4">Tracking dynamic paid cashflows timeline</div>
          </div>
        ...
        <Card className="p-5 flex flex-col justify-between">
          <div>
            <div className="text-sm font-semibold flex items-center gap-1.5"><Store className="h-4.5 w-4.5 text-primary" /> Top Vendors Performance</div>
            <div className="text-xs text-muted-foreground mb-4">Aggregated cashflows per vendor profile</div>
          </div>
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={vendorPerformance}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" />
                <XAxis dataKey="businessName" fontSize={10} stroke="var(--color-muted-foreground)" />
                <YAxis fontSize={11} stroke="var(--color-muted-foreground)" tickFormatter={(v) => `${v / 1000}k`} />
                <Tooltip {...tooltipStyle} />
                <Bar name="Revenue" dataKey="revenue" fill="var(--color-chart-4)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      {/* Grids containing top products and top vendors tables */}
      <div className="grid gap-4 grid-cols-1 xl:grid-cols-2">
        ...
        <Card className="p-5 space-y-4">
          <div className="font-semibold text-sm">Vendor Rankings</div>
          <DataTable columns={vendorColumns} data={vendorPerformance} pageCount={1} pageIndex={0} pageSize={5} />
        </Card>
      </div>
    </div>
  );
