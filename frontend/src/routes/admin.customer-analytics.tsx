...
import { Users, IndianRupee, Receipt, AlertTriangle, Store, Plus, FileBarChart, Download, Search, RefreshCw, Sparkles, TrendingUp, Calendar, MapPin, BadgeCheck, HelpCircle } from "lucide-react";
...
export const Route = createFileRoute("/admin/customer-analytics")({
  ...
  component: AdminCustomerAnalytics,
});

function AdminCustomerAnalytics() {
  ...
  const handleExportCSV = () => {
    try {
      ...
      link.setAttribute("download", `ShopSense_CustomerAnalytics_${new Date().toISOString().split("T")[0]}.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      toast.success("CSV export downloaded successfully");
    } catch (err) {
      toast.error("Failed to export analytical data to CSV");
    }
  };

  // Recharts formatted maps
  ...
  const topCustomersChartData = (data.topCustomers || []).slice(0, 5).map((tc: any) => ({
    name: tc.name,
    revenue: tc.revenue,
  }));
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader
        ...
        description="Observe cohorts segmentation, dynamic growth scales, retention lines, and value averages."
        actions={
          <>
      ...
      <Card className="p-4 bg-muted/20 border-dashed space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-end pt-2 border-t border-border/50">
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
              <Calendar className="h-3.5 w-3.5" /> Start Registration Date
            </label>
            <Input
              type="date"
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
              <Calendar className="h-3.5 w-3.5" /> End Registration Date
            </label>
            <Input
              type="date"
              value={localEndDate}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-8">
        ...
        <StatCard label="Active directory" value={num(cards.activeCustomers || 0)} delta={2.1} icon={BadgeCheck} tone="accent" />
        <StatCard label="New customers (30d)" value={num(cards.newCustomers || 0)} delta={12.4} icon={Plus} tone="primary" />
        <StatCard label="Repeat customers" value={num(cards.repeatCustomers || 0)} delta={8.5} icon={Users} tone="accent" />
        <StatCard label="Total Revenue" value={inr(cards.totalCustomerRevenue || 0)} delta={18.6} icon={IndianRupee} tone="primary" />
        ...
        <StatCard label="Top Buyer" value={cards.highestSpendingCustomer || "N/A"} icon={Sparkles} tone="info" />
      </div>
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-3">
        ...
        <Card className="p-5 flex flex-col justify-between">
          <div>
            <div className="text-sm font-semibold">Segmentation distribution</div>
            <div className="text-xs text-muted-foreground mb-4">Unique customers by category tier</div>
          </div>
        ...
        {/* Daily Purchases Area */}
        <Card className="p-5 lg:col-span-2 flex flex-col justify-between">
          <div>
            <div className="text-sm font-semibold">Daily Purchase Trend</div>
            <div className="text-xs text-muted-foreground mb-4">Customer daily order count volume & total revenue timeline</div>
          </div>
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.dailyPurchaseTrend || []} margin={{ left: -10, right: 10, top: 5 }}>
                <defs>
                ...
                <XAxis dataKey="day" tickLine={false} fontSize={10} stroke="var(--color-muted-foreground)" />
                <YAxis tickLine={false} fontSize={11} stroke="var(--color-muted-foreground)" tickFormatter={(v) => v >= 1000 ? `${(v / 1000).toFixed(1)}k` : v} />
                <Tooltip {...tooltipStyle} />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Area type="monotone" name="Revenue" dataKey="revenue" stroke="var(--color-chart-1)" strokeWidth={2} fill="url(#revenueG)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-3">
        ...
        <Card className="p-5 lg:col-span-3 flex flex-col justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground mb-4">Cumulative daily registration curve and new sign-ups</div>
          </div>
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data.dailyGrowth || []} margin={{ left: -10, right: 10, top: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" />
                <XAxis dataKey="day" tickLine={false} fontSize={10} stroke="var(--color-muted-foreground)" />
      ...
      <div className="grid gap-4 grid-cols-1 xl:grid-cols-3">
        ...
        <Card className="p-5 xl:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold">Top Customers Directory</div>
              <div className="text-xs text-muted-foreground">List of customers with highest spending totals</div>
            </div>
          </div>
