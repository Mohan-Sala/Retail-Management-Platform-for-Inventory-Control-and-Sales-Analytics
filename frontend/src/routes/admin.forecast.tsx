...
import { Download, RefreshCw, Sparkles, TrendingUp, AlertTriangle, BatteryMedium, CheckCircle, Package } from "lucide-react";
...
// Search schema definition for TanStack Router query parameter validation
...
function AdminForecastPage() {
  ...
  // Load vendors list for filter options
  useEffect(() => {
  ...
  // Trigger manual router loader refresh
  const handleRefresh = async () => {
    toast.promise(router.invalidate(), {
      loading: "Refreshing forecasting models...",
      success: "Forecast models synchronized successfully!",
      error: "Failed to reload forecast configurations.",
    });
  };
  ...
  const handleExportCSV = () => {
    try {
      ...
      link.setAttribute("download", `ShopSense_Forecast_${searchParams.forecastDays}Days_${new Date().toISOString().split("T")[0]}.csv`);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Dynamic sales rate modeling, risk alerts, and automated stock requirements planning."
        actions={
          <>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Marketplace Daily Velocity" value={`${avgDailySales} units`} icon={Sparkles} tone="info" />
      </div>

      {/* Advanced Filters and Period Config toolbar */}
      <div className="flex flex-wrap items-center gap-3 p-4 rounded-xl border bg-card text-card-foreground shadow-sm">
        ...
        {/* Vendor Filter */}
        <div className="w-[160px]">
          <Select value={searchParams.vendor} onValueChange={(val) => updateQuery({ vendor: val })}>
            <SelectTrigger>
              <SelectValue placeholder="All Vendors" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Vendors</SelectItem>
              {vendors.map(v => <SelectItem key={v.id} value={v.id}>{v.businessName}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        {(searchParams.search || searchParams.category !== "all" || searchParams.vendor !== "all" || searchParams.status !== "all" || searchParams.forecastDays !== 30 || searchParams.historyDays !== 30) && (
      ...
      {forecasts.length > 0 ? (
        <DataTable
          columns={columns}
          data={forecasts}
          searchKeys={[]} // Disabled client search since filters are bound in url/backend
          searchPlaceholder=""
        />
      ) : (
