...
export const Route = createFileRoute("/vendor/products/$id")({
  loader: async ({ params }) => {
    try {
...
function VendorProductDetail() {
  ...
  return (
    <div>
      <BackLink to="/vendor/products" label="Back to products" />
      <PageHeader title={p.name} description={`${p.category} · ${p.sku}`}
        actions={<Button asChild><Link to="/vendor/products/$id/edit" params={{ id: p.id }}><Pencil className="mr-1.5 h-4 w-4" /> Edit</Link></Button>} />
